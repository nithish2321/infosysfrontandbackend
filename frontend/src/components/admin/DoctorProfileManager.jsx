...
import { 
  ChevronDown, ChevronUp, Save, Lock, User, Briefcase, 
...
const SECTION_CONFIG = [
  ...
  { id: "schedule", label: "5. Schedule & Availability", icon: <Calendar size={18}/>, adminOnly: true },
...
const DoctorProfileManager = ({ profile, role, onSave, onCancel }) => {
  ...
  // ✅ NEW: Smart Field Renderer
  const renderField = (sectionId, key, value, canEdit) => {
    const lowerKey = key.toLowerCase();
    const label = key.replace(/([A-Z])/g, " $1").trim(); // Add spaces to CamelCase
    ...
    if (lowerKey.includes("status") && !lowerKey.includes("marital")) {
        return (
          <select {...commonProps}>
    ...
    // 2. DATES (DOB, Expiry, Joining)
    if (lowerKey.includes("date") || lowerKey.includes("dob") || lowerKey.includes("valid") || lowerKey === "start" || lowerKey === "end") {
      return <input type="date" {...commonProps} />;
    }

    // 3. TEXT AREAS (Address, Bio, Notes)
    if (lowerKey.includes("address") || lowerKey === "bio" || lowerKey.includes("description") || lowerKey.includes("notes")) {
      return <textarea {...commonProps} rows={3} className={`${commonProps.className} resize-none`} />;
    }

    // 4. NUMBERS (Salary, Fees, Experience)
    if (lowerKey.includes("salary") || lowerKey.includes("fee") || lowerKey.includes("experience") || lowerKey.includes("year")) {
